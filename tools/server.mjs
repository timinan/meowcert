...
const TOOLS = {
  ...
  economy: {
    ...
    // No save endpoint — placeholder path keeps the TOOLS table uniform
    // (indexHtml() renders path.relative(savePath) for every entry).
    savePath: path.join(TOOL_DIR, 'economy', 'placeholder.json'),
  ...
  'visit-post-preview': {
    ...
    // No save endpoint — placeholder path keeps the TOOLS table uniform
    // (indexHtml() renders path.relative(savePath) for every entry).
    savePath: path.join(TOOL_DIR, 'visit-post-preview', 'placeholder.json'),
...
function indexHtml() {
  const rows = Object.entries(TOOLS)
    .map(
      ([key, t]) => {
        const saveLine = t.savePath
          ? `POST /save/${key} → ${path.relative(PROJECT_ROOT, t.savePath)}`
